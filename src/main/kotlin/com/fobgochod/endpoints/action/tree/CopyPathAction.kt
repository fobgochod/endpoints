...
class CopyPathAction : EndpointsAction() {

    init {
        templatePresentation.icon = AllIcons.Actions.Copy
        templatePresentation.text = message("action.editor.copy.path.text")
    }

    override fun actionPerformed(e: AnActionEvent, project: Project) {
        val selectPath = PsiFileUtils.getSelectedPath(project)
        if (selectPath is EndpointNode) {
            PsiFileUtils.copyPath(project, selectPath.source, false)
        }
    }
